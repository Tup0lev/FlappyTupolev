...
function setLoadingText(percent) {
  ...
  if (percent <= 20)
    loadingText.setText(
    '正在唤醒图酱: %s %'.replace('%s', percent)
    );
  if (percent >20 && percent < 40)
    loadingText.setText(
    '正在加注燃油: %s %'.replace('%s', percent)
    );
  if (percent >40 && percent < 60)
    loadingText.setText(
    '正在盈利盈利: %s %'.replace('%s', percent)
    );
  if (percent >60 && percent < 80)
    loadingText.setText(
    '启动发动机: %s %'.replace('%s', percent)
    );
  if (percent >80)
    loadingText.setText(
    '赶紧关注斯拉夫虚拟偶像你这个臭ДД: %s %'.replace('%s', percent)
    );
}
...
function showScore() {
  scoreText.setText(
    '营业 %s 次'.replace('%s', global.score)
  );
  bestScoreText.setText(
    'BEST %s 次'.replace('%s', global.bestScore)
  );
